...
Made a form with an onSubmit handler and targeted the input by using
a direct id on the input that matches the labels htmlFor. This allows for accessibility in 
screen readers. To grab that ID, we used ```event.target.elements.usernameInput.value```, where usernameInput was the
custom ID we specified to both htmlFor and ID.
Without ```e.preventDefault()```, the browser will submit the form in the form of a GET request.

React.useRef is a special React hook.

For error checking the submit, we used useState to generate an array that contains ```[error, setError]```
Error is the current state and setError is the error message we'd like to display.
and set the intial state to null. We created a function called handleChange where we targeted the value from the event
(either by deconstruction using ```const {value} = e.target``` or ```const value = e.target.value```). To
set the error, we called setError and passed a ternary operator.

For handling the change, we had to added a onChange prop to the input form and point it to our handleChange function.

Used ```disabled={error}``` to disable the button by passing the event state.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=danaimone%40gmail.com).
